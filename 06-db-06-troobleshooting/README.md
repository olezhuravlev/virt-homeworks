...
Помимо **физического наращивания памяти** в серверах следует оптимизировать использование памяти БД PostgreSQL с помощью следующих настроек:

| Параметр             | Примечание                                                              | По умолчанию | Рекомендуемая доля от RAM                                               | 
|----------------------|-------------------------------------------------------------------------|--------------|-------------------------------------------------------------------------|
| `shared_buffers`       | кэширования                                                             | 128MB        | 15%-40%                                                                  | 
| `work_mem`             | внутренние операции сортировки и хэширования                            | 4MB          | 25% / max_connections                                                   |
| `maintenance_work_mem` | операции поддержки (vacuum, create index, alter table, add foreign key) | 64MB         | 5%  (н.м. work_mem)                                                     |
| `effective_cache_size` | кеширование диска ОС и СУБД                                             | 4GB          | 50%                                                                     |
| `temp_buffers`         | буфер временных таблиц (для каждой сессии)                              | 8MB          | Подбирается в зависимости от использования временных таблиц в запросах. |
| `max_connections`      | Макс.число конкурентных соединений к БД                                 | 100          | Подбирается в зависимости от нагрузки.                                  |

Допустим, что на сервере физически доступно 128GB памяти, соответственно, установим в файле `postgres.conf` следующие значения:
...
Пераметры `temp_buffers` и `max_connections` следует подобрать экспериментально.

После изменение настроек памяти сервер необходимо перезапустить!
